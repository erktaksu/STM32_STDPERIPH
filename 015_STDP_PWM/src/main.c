...
void TIM_Config()
{
	...
	TIM_InitSturct.TIM_Period=9999;
	TIM_InitSturct.TIM_Prescaler=8399;
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  TIM_OC1PreloadConfig(TIM4,TIM_OCPreload_Enable); //tim4 ch1 kanal aktif
	  //PD13

	  TIMOC_InitStruct.TIM_Pulse=7499;
	  TIM_OC2Init(TIM4,&TIMOC_InitStruct);
	  TIM_OC2PreloadConfig(TIM4,TIM_OCPreload_Enable); //tim4 ch2 kanal aktif

	  //PD14

	  TIMOC_InitStruct.TIM_Pulse=4999;
	  TIM_OC3Init(TIM4,&TIMOC_InitStruct);
	  TIM_OC3PreloadConfig(TIM4,TIM_OCPreload_Enable); //tim4 ch3 kanal aktif

	  //PD15

	  TIMOC_InitStruct.TIM_Pulse=2499;
	  TIM_OC4Init(TIM4,&TIMOC_InitStruct);
	  TIM_OC4PreloadConfig(TIM4,TIM_OCPreload_Enable); //tim4 ch4 kanal aktif
  }
}
